    ...
    suspend fun getLyrics(mediaMetadata: MediaMetadata): LyricsWithProvider {
        ...
        val result = withTimeoutOrNull(MAX_LYRICS_FETCH_MS) {
                ...
                try {
                    ...
                    when {
                        result?.isSuccess == true -> {
                            val rawLyrics = result.getOrNull().orEmpty()
                            val filteredLyrics = LyricsUtils.filterLyricsCreditLines(rawLyrics)
                            if (filteredLyrics.isNotBlank() && filteredLyrics != LYRICS_NOT_FOUND) {
                                val isTimeSynced = filteredLyrics.lines().any { line ->
                                    val t = line.trim()
                                    t.startsWith("[") || t.startsWith("<")
                                }
                                if (isTimeSynced) {
                                    Timber.tag("LyricsHelper").i("Successfully got synced lyrics from ${provider.name}")
                                    return@withTimeoutOrNull LyricsWithProvider(filteredLyrics, provider.name)
                                } else if (unsyncedFallback == null) {
                                    Timber.tag("LyricsHelper").i("Got unsynced lyrics from ${provider.name}, searching for synced fallback...")
                                    unsyncedFallback = LyricsWithProvider(filteredLyrics, provider.name)
                                }
                            }
                        }
                        result == null -> {
            ...
            if (unsyncedFallback != null) {
                Timber.tag("LyricsHelper").i("Returning unsynced fallback lyrics from ${unsyncedFallback.provider}")
                return@withTimeoutOrNull unsyncedFallback
            }
